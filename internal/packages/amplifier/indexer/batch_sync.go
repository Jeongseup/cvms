...
func (indexer *AmplifierIndexer) batchSync(lastIndexPointerHeight, newIndexPointerHeight int64) (
	/* new index pointer */ int64,
	/* error */ error,
) {
	if lastIndexPointerHeight >= indexer.Lh.LatestHeight {
		indexer.Infof(
			"current height is %d and latest height is %d both of them are same, so it'll skip the logic",
			lastIndexPointerHeight, indexer.Lh.LatestHeight,
		)
	...
	endHeight := indexer.Lh.LatestHeight

	// set limit at end-height in this batch sync logic
	if (indexer.Lh.LatestHeight - newIndexPointerHeight) > indexertypes.BatchSyncLimit {
		endHeight = newIndexPointerHeight + indexertypes.BatchSyncLimit
		indexer.Debugf("by batch sync limit, end height will change to %d", endHeight)
	}
	...
	// txSummaryList := make(map[int64]types.TxSearchSummary, 0)
	// for height := startHeight; height <= endHeight; height++ {
	// 	wg.Add(1)
	// 	height := height

	// 	go func(ch chan helper.Result) {
	// 		defer helper.HandleOutOfNilResponse(indexer.Entry)
	// 		defer wg.Done()

	// 		txsHashes, txsEvents, err := api.GetTxSearch(indexer.CommonClient, height)
	// 		if err != nil {
	// 			indexer.Errorf("failed to call at %d height data, %s", height, err)
	// 			ch <- helper.Result{Item: nil, Success: false}
	// 			return
	// 		}

	// 		ch <- helper.Result{
	// 			Item: types.TxSearchSummary{
	// 				BlockHeight: height,
	// 				TxHashes:    txsHashes,
	// 				TxsEvents:   txsEvents,
	// 			},
	...
	// for r := range ch {
	// 	if r.Success {
	// 		item := r.Item.(types.TxSearchSummary)
	// 		txSummaryList[item.BlockHeight] = item
	// 		continue
	// 	}
	// 	errorCount++
	// }
	...
	// if errorCount > 0 {
	// 	return lastIndexPointerHeight, errors.Errorf("failed to collect batch block result data, total errors: %d", errorCount)
	// }

	// makeDataList(summaryList)
	...
	/*
		...
		for _, blockResult := range blockResultSummaryList {
			for _, event := range blockResult.TxsEvents {
				event.TypeName == ""
			}

			// check proposer address
			_, exist := indexer.Vim[blockInfo.BlockProposerAddress]
			if !exist {
			...
			for _, validator := range blockInfo.CosmosValidators {
				_, exist := indexer.Vim[validator.Address]
				if !exist {
					indexer.Debugf("the miss validator %s/%s address isn't in current validator info table, it will be gonna added into the table", validator.Address, validator.Pubkey)
		...
		if isNewValidator {
			newValidatorInfoList, err := function.MakeValidatorInfoList(indexer.CommonApp,
				indexer.ChainID, indexer.ChainInfoID,
				indexer.ChainName, indexer.IsConsumer,
